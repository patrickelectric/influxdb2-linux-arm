...
* Provide pre-build DEB and RPM packages for ARM 32-bit.
* Provide a means to easily build you own packages without the need of in depth knowledge.
* Automate the build as much as possible.
...
* [Buildx](https://docs.docker.com/buildx/working-with-buildx/) plugin for docker installed

## For the gullible and lazy

You can download the pre-build packages from the [Download](https://bitbucket.org/choekstra/influxdb2-linux-arm/downloads/) page.
Checksum files are provided. 

> Disclaimer: Although I did not manipulate the code in anyway provide by InfluxData, it is best practice not to trust sources like these. I highly recommend to build the packages yourself when able.

## Build your own packages!

To build your own packages, make sure you have the prerequisites in place. Once you have setup your Linux OS with Docker.io, clone this repository by executing `git clone https://choekstra@bitbucket.org/choekstra/influxdb2-linux-arm.git`.

### Build Docker image

Using [buildx](https://docs.docker.com/buildx/working-with-buildx/) is highly recommended, but not required. This image contains everything that is needed to cross-compile Influxdb2 and to build the DEB and RPM packages. Influxdb2 is build using multiple program languages e.g., GOLANG and RUSTC.

```bash
$ cd influxdb
$ ./bake_image.sh
buildx plugin is installed, continuing using buildx:
[+] Building 186.2s (15/15) FINISHED
 ...
 => [10/10] COPY [build_package.sh, col_table, influxdb2_nfpm.yml, influxdb2-client_nfpm.yml, .]               0.0s
 ...
 => => naming to docker.io/library/influxdb2builder                                                            0.0s
 
 $ docker images
 REPOSITORY         TAG       IMAGE ID       CREATED              SIZE
influxdb2builder   latest    394bec3ca626   About a minute ago   2.3GB
```

### Build Influxdb2 and Client packages

Since Influxdb2 v2.0.9 the client `influx` will be maintained in a separate repository and package. Therefore this project builds a package for Influxdb and one for the client.
...
### PREPERATIONS ###
  ...
  [i] Initialized log file /build_output/influxdb2-build-20220125.log...
...
Files can be found in: /home/<your home>/influxdb2/build_output

$ cd /home/<your home>/influxdb2/build_output
```
...
The image and build has been fully automated and provides the following features:

**Build Features**

* Build DEB packages
* Build RPM packages
* Build TAR archive containing binaries only
...
* Fully automated cross compile build of binaries
* Fully automated DEB, RPM and TAR archive build
